...
- A general edge extraction algorithm is employed. (Same algorithm as traditionally used for stencil shadows) Once all unique edges are found, all 
  edges that connect to only one triangle are extracted.
...
public class MeshExtrusion
{
    ...
    public static void ExtrudeMesh(Mesh srcMesh, Mesh extrudedMesh, Matrix4x4[] extrusion, Edge[] edges, bool invertFaces)
    {
        ...
        for (int i = 0; i < extrusion.Length; i++)
        {
            ...
            {
                /** Le code n'est pas de moi mais j'ai dû modifier cette partie là.
                 * Ces uvs causaient un bug sur les textures des bâtiments. 
                 * En prenant la moyenne, le problème était résolu, on n'avait plus de distorsion de texture.
                 */
                ...
                uvs[v + 0] = new Vector2((inputUV[e.vertexIndex[0]].x + inputUV[e.vertexIndex[0]].y)/2 , vcoord);
                uvs[v + 1] = new Vector2((inputUV[e.vertexIndex[1]].x + inputUV[e.vertexIndex[1]].y)/2, vcoord);
